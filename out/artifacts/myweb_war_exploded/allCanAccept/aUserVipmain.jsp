...
<%@ page import="cn.edu.lingnan.dto.VipDTO" %>
...
<head>
    ...
    <script>
        ...
        function one(phone, name) {
            document.getElementById("phoneF").value = phone;
            document.getElementById("nameF").value = name;
        ...
        function check() {
            var regm = /[0-9]+$/;
            if (form1.phoneF.value == "" || form1.phoneF.value == null) {
                alert("电话号码为空...");
                form1.phoneF.focus();
                return false;
            }
            if (!form1.phoneF.value.match(regm)) {

                alert("电话号码输入非法字符...");
                form1.phoneF.focus();
                return false;
            }
            if (form1.phoneF.value.length != 11 ) {
        ...
        function three(name) {
            ...
            if (massage) {
                document.getElementById("vipDelete").value = name;
                formD.submit();
        ...
        function check2() {
            var regm = /[0-9]+$/;
            if (formInsert.phoneI.value == "" || formInsert.phoneI.value == null) {
                alert("电话号码为空...");
                formInsert.phoneI.focus();
                return false;
            }
            if (!formInsert.phoneI.value.match(regm)) {

                alert("电话号码输入非法字符...");
                formInsert.phoneI.focus();
                return false;
            }
            if (formInsert.phoneI.value.length != 11 ) {

                alert("电话号码为11位数字...");
                formInsert.phoneI.focus();
                return false;
            }
            if (formInsert.usernameI.value == "" || formInsert.usernameI.value == null) {
                alert("用户名为空...");
...
<div class="container-scroller">
                        ...
                        <div class="card">
                            <div class="card-body">
                                <form class="form-inline" accept-charset="GB18030" name="formS"
                                      action="searchVipServlet" method="post">
                                    <div class="form-group col-4">
                                        <div class="input-group">
                                            <input type="text" id="SearchTemp" name="SearchTemp" class="form-control"
                                                   placeholder="电话号码或姓名" aria-label="Recipient's username"
                                    ...
                                    <div class="form-group col-4">
                                        <button type="button" class="btn btn-outline-primary btn-md"
                                                onclick="window.location.href='aUservipServlet'">全部寻找
                                        </button>
                                    </div>
                                    <%
                                        }
                                ...
                                <table class="table table-bordered" style="text-align: center" border="5">
                                    ...
                                    <%
                                        if (session.getAttribute("authority") != null) {
                                            ...
                                            Vector<VipDTO> v = (Vector<VipDTO>) session.getAttribute("AllVip");
                                            System.out.println(v.size());
                                            Iterator<VipDTO> it = v.iterator();
                                            VipDTO u = new VipDTO();
                                            while (it.hasNext()) {
                                                u = it.next();
                                    %>
                                    <tr class="table-danger">
                                        ...
                                        <td style="width: 204px;">
                                            <input type="button" class="btn btn-outline-primary btn-sm"
                                                   onclick='one("<%=u.getPhone()%>","<%=u.getName()%>") ' value="修改">
                                            </input>
                                            <label style="width: 10px;"></label>
                                            <button class="btn btn-outline-dark  btn-sm"
                                                    onclick='three("<%=u.getName()%>") '>删除
                                            </button>
                                        </td>
                                    </tr>
                                    <%
                            ...
                            <form name="formD" action="deleteVipServlet">
                                <input type="hidden" id="vipDelete" name="vipDelete">
                            </form>
    ...
    <div id="light" class="white_content">
        <div class="card-body">
            <form class="forms-sample" accept-charset="GB18030" name="form1" action="changeVipServlet" method="post">
                <div class="form-group" style="margin-bottom: 0px;">
                    <label for="phoneF">电话</label>
                    <input style="padding-top: 5px;padding-bottom: 5px;" class="form-control" type="label" id="phoneF"
                           name="phoneF"/>
                </div>
                <div class="form-group" style="margin-top:5px; margin-bottom: 5px;">
                    <label for="nameF">姓名</label>
                    <input style="padding-top: 5px;padding-bottom: 5px;" class="form-control" type="label" id="nameF"
                           name="nameF" readonly="readonly"/>
                </div>
    ...
    <div id="Insertdiv" class="white_content">
        <div class="card-body">
            <form class="forms-sample" accept-charset="GB18030" name="formInsert" action="insertVipServlet"
                  method="post">
                <div class="form-group" style="margin-bottom: 0px;">
                    <label for="phoneI">电话</label>
                    <input style="padding-top: 5px;padding-bottom: 5px;" class="form-control" type="label" id="phoneI"
                           name="phoneI"/>
                </div>
                <div class="form-group" style="margin-top:5px; margin-bottom: 5px;">
