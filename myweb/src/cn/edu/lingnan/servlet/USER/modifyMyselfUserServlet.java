...
public class modifyMyselfUserServlet  extends HttpServlet {
    ...
    { request.setCharacterEncoding("GB18030");
        ...
        if(bool){
            ...
            if(authority.compareTo("su")==0)
                response.getWriter().print( "<script>alert(\"输入的值出现错误，请重新操作!\");window.location.href='../admin/adminmain.jsp'</script>");
            else response.getWriter().print( "<script>alert(\"输入的值出现错误，请重新操作!\");window.location.href='sales.jsp'</script>");
